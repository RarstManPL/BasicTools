...
public class SocialSpyCommand extends Command {
    ...
    public SocialSpyCommand(final Configuration.BasicCommand basicCommand) {
    ...
    public void onExecute(final CommandSender commandSender, final String[] args) {
        ...
        if (isSocialspying == null) {
            ChatUtil.sendMessage(commandSender, this.messages.getMessage("TurnOptionNotExist"));
            return;
        }
        final String variableStatus = isSocialspying ? this.messages.getMessage("On") : this.messages.getMessage("Off");
        userCache1.setSocialspying(isSocialspying);
        ChatUtil.sendMessage(commandSender, this.messages.getMessage("SocialSpyStatusChanged"),
        ...
        ChatUtil.sendMessage(player1, this.messages.getMessage("SocialSpyStatusChangedInfo"), "{status}", variableStatus);
    }

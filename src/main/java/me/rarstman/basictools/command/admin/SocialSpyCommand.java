...
public class SocialSpyCommand extends CommandProvider {

    private final BasicToolsMessages messages;
    private final UserManager userManager;

    public SocialSpyCommand() {
        super(ConfigManager.getConfig(BasicToolsCommands.class).socialSpyCommandData, "basictools.command.socialspy", false);
    ...
    public void onExecute(final CommandSender commandSender, final String[] args) {
        ...
        final boolean isSocialspying = args.length > 0 ? BooleanUtil.stringStatusToBoolean(args[0]) : !userCache1.isSocialspying();
        final String variableStatus = isSocialspying ? this.rarstAPIMessages.on_ : this.rarstAPIMessages.off_;

        userCache1.setSocialspying(isSocialspying);
        this.messages.socialSpyStatusChanged.send(commandSender,
        ...
        this.messages.socialSpyStatusChangedInfo.send(player1, "{STATUS}", variableStatus);
    }
